...
def get_vectorstore(text_chunks):
    try:
        ...
        if openai_api_key:
            ...
            raise Exception("Failed to initialize embeddings. Please check your dependencies or provide an OpenAI API key.")
...
def get_conversation_chain(vectorstore):
    if openai_api_key:
        llm = ChatOpenAI(temperature=0.23)
        print("Using OpenAI for chat model")
    else:
        # Use a local model as fallback
        from langchain_community.llms import HuggingFacePipeline
        model_id = "google/flan-t5-base"  # A smaller model that should run on most systems
